...
description: Install, verify or remove the shell integration this plugin ships — `super-claude`
  ...
  then repairs it. The SessionStart hook already does this at every launch; this skill is for
  the first install, for a machine where the hook has not run yet, and for diagnosing.
when_to_use: Use right after installing kodflow-shell, on a machine where `super-claude` is not
...
model: haiku
allowed-tools:
- Bash(${CLAUDE_PLUGIN_ROOT}/bin/kodflow-shell-setup:*)
- Bash(command -v:*)
- Bash(type:*)
- Bash(readlink:*)
- Bash(ls:*)
