...
description: Draw the kodflow workflow as an ASCII diagram — the whole map, the nominal path
  ...
  leaves behind, so the diagram doubles as the prompt for what to run next.
when_to_use: Use to see how the skills fit together — at the start of a session, when you know
  what you want but not which skill opens it, when handing the setup to someone else, or when
  a README needs the map. Also to check the diagram still matches the installed plugins.
argument-hint: '[<skill>] [--path|--artifacts|--hooks|--agents|--all] [--ascii] [--check]'
model: haiku
allowed-tools:
- Read
- Glob
- Bash(bash "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}"/skills/flow/scripts/flow-inventory.sh:*)
---
...
```bash
bash "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}"/skills/flow/scripts/flow-inventory.sh --names-only
```
