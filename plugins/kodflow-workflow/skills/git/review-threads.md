...
| **Unclear** | `AskUserQuestion`; never resolve on the user's behalf |
...
The GitHub tool (`mcp__github__*`) is preferred where it covers the call:
...
Approval is a separate gate: `mcp__gitlab__approve_merge_request` only when the
