...
> Implementation: `"${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}"/skills/_shared/scripts/team-mode-primitives.sh`
...
Source `"${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}"/skills/_shared/scripts/team-mode-primitives.sh` and call `detect_runtime_mode`. Every team-aware skill starts with:
...
The pilot `/review` skill is the reference implementation. When migrating a new skill, read `~/.claude/skills/review/dispatch.md` first.
...
Plan: `$HOME/.claude/plans/agent-teams-refactor-v3.md`
