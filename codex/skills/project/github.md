...
MCP-first: prefer the GitHub tool for API calls, fall back to `gh` when a tool
...
2. Otherwise call a multiple-choice question to the user **once**:
...
- Several → a multiple-choice question to the user with each login as an option.
...
|---------|-------|---------|
| `gh: command not found` | `gh` absent | fall back to the GitHub tool; if that is also unavailable, run with `--no-github` and tell the user |
