...
metadata:
  short-description: Open a piece of feature work and keep its trail
  generated-from: plugins/*/skills/feature
...
Destination, first that answers:
  note store (the note store) → GitLab issue → GitHub issue → .claude/issues/
...
a multiple-choice question to the user for the missing pieces — one question, options drawn from what
