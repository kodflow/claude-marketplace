...
description: 'Workflow Git Automation with RLM decomposition. Handles branch management, conventional
  ...
  CI checks. Supports GitHub (PRs) and GitLab (MRs) - auto-detected from git remote. Use when
  to commit, open a PR/MR, watch CI to green, or merge. Handles branch naming, conventional
  commits, and the pre-push quality gate.'
metadata:
  short-description: Workflow Git Automation with RLM decomposition
  generated-from: plugins/*/skills/git
...
|---------|--------|--------|
| `--commit` | Branch, commit, push, PR/MR | `identity.md` → `commit.md` |
| `--watch` | Monitor & fix until green | `watch.md` |
| `--merge` | Merge with CI validation | `merge.md` |
...
|--------|------------------|----------------|
| Create branch | the GitHub tool | `git checkout -b` |
| Create PR | the GitHub tool | `gh pr create` |
| List PRs | the GitHub tool | `gh pr list` |
| View PR | the GitHub tool (method: get) | `gh pr view` |
| CI Status | the GitHub tool (method: get_status) | _No fallback (MCP-only)_ |
| Merge PR | the GitHub tool | `gh pr merge` |
...
| Create MR | the GitLab tool | `glab mr create` |
| List MRs | the GitLab tool | `glab mr list` |
| View MR | the GitLab tool | `glab mr view` |
| CI Status | the GitLab tool | `glab ci status` |
| Merge MR | the GitLab tool | `glab mr merge` |
...
1. **Read `identity.md`** — Phase 0.5: Validate git identity & GPG
   - Skip if `--skip-identity` flag passed
2. **Read `commit.md`** — Phases 2.0-7.0: Full commit workflow
...
1. **Read `merge.md`** — Full merge workflow
2. **Read `guardrails.md`** — Safety rules (referenced throughout)
...
1. **Read `watch.md`** — Full watch workflow
...
**Read `guardrails.md` for the complete safety rules.**
