...
peek_workflow:
  1_collect_all:
    ...
    command: "bash "${CODEX_HOME:-$HOME/.codex}"/skills/_shared/scripts/git-peek.sh"
...
incremental_quality:
  ...
  execution:
    command: "bash "${CODEX_HOME:-$HOME/.codex}"/skills/_shared/scripts/pre-commit-quality.sh main"
...
| Type | Action | Example |
|------|--------|---------|
| Real secret (token, prod password) | **BLOCK the commit** | `ghp_abc123...`, `postgres://user:<actual-password>@prod/db` |
