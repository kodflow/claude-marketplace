...
description: 'DevContainer Environment Update from official template. Profile-aware: auto-detects
  ...
  improvements. Use when to sync a project''s devcontainer with the upstream template — checking
  what drifted, or applying the update.'
metadata:
...
| `commands` | `~/.codex/skills/` | Slash commands |
...
Components:
  ...
  mcp-fragments MCP server fragments (context7, project-linter)
...
| Phase | Action | Module |
|-------|--------|--------|
| 1.0 | Environment detection (container vs host) | Read `detect.md` |
| 1.5 | Profile detection (devcontainer vs infrastructure) | Read `detect.md` |
| 2.0 | Version check (peek) | Read `diff.md` |
| 3.0 | Download tarballs (git tarball) | Read `diff.md` |
| 4.0 | Extract & apply components | Read `apply.md` |
| 5.0 | Orchestration & compose merge | Read `apply.md` |
| 6.0 | Hook synchronization | Read `validate.md` |
| 7.0 | Script validation | Read `validate.md` |

**IF `the arguments` contains `--help`**: Display the help above and STOP.
