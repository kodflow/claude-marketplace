...
description: 'Systematic, root-cause-first debugging. Enforces a no-fix-without-root-cause
  ...
  the architecture after repeated failed fixes instead of thrashing. Use when for a bug whose
  cause is not obvious — a flaky test, an intermittent failure, a wrong result with no exception.
  Not for a compile error or a stack trace that already names the line.'
metadata:
  short-description: Systematic, root-cause-first debugging
  generated-from: plugins/*/skills/debug
