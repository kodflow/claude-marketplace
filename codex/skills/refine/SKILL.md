...
description: Skills Architecture v1.6 — proof-bearing goal contract generator with three entry
  ...
  viable length that preserves the contract, never pads to fill the budget. Use when to turn
  a plan or a loose description into a /goal contract with binary acceptance criteria paired
  1:1 with verifiers, so the goal state cannot be reached by a vague claim of success.
metadata:
  short-description: Skills Architecture v1
  generated-from: plugins/*/skills/refine
...
Read `auto.md`. AUTO picks 4-critical-lenses
...
Read `dispatch.md`. For each lens, call
